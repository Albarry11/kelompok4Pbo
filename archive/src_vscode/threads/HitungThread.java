...
/**
 * Class HitungThread - Worker thread untuk perhitungan paralel
 * Data random > 99.000 via Math.random()
 */
public class HitungThread implements Runnable {
    
    private String namaBentuk;
    private String tipeBentuk;
    ...
    public void run() {
        ...
        try {
            ...
            if (nextThread != null && nextThread.isAlive()) {
                appendLog("[" + tn + "]  >> INTERRUPT dikirim ke " + nextThread.getName());
            ...
            for (int i = dataSelesai; i < jumlahData; i++) {
                ...
                switch (tipeBentuk) {
                    case "LAYANG":
                        LayangLayang ll = new LayangLayang(d1, d2, sisiA, sisiB);
                        synchronized (this) { totalLuas += ll.hitungLuas(); totalKeliling += ll.hitungKeliling(); }
                        break;
                    case "LIMAS":
                        LimasLayangLayang limas = new LimasLayangLayang(d1, d2, sisiA, sisiB, tinggi);
                        synchronized (this) { totalLuas += limas.hitungLuas(); totalVolume += limas.hitungVolume(); totalLuasPermukaan += limas.hitungLuasPermukaan(); }
                        break;
                    case "PRISMA":
                        PrismaLayangLayang prisma = new PrismaLayangLayang(d1, d2, sisiA, sisiB, tinggi);
                        synchronized (this) { totalLuas += prisma.hitungLuas(); totalVolume += prisma.hitungVolume(); totalLuasPermukaan += prisma.hitungLuasPermukaan(); }
                        break;
                }
                dataSelesai = i + 1;
            }
    ...
    private void logHasil(String tn) {
        appendLog("[" + tn + "]  HASIL  |  Total Luas           = " + String.format("%,.2f", totalLuas));
        if (tipeBentuk.equals("LAYANG")) {
            appendLog("[" + tn + "]         |  Total Keliling       = " + String.format("%,.2f", totalKeliling));
            appendLog("[" + tn + "]         |  Rata-rata Luas      = " + String.format("%,.2f", totalLuas / jumlahData));
            appendLog("[" + tn + "]         |  Rata-rata Keliling  = " + String.format("%,.2f", totalKeliling / jumlahData));
        }
        if (tipeBentuk.equals("LIMAS") || tipeBentuk.equals("PRISMA")) {
            appendLog("[" + tn + "]         |  Total Volume        = " + String.format("%,.2f", totalVolume));
            appendLog("[" + tn + "]         |  Total LP            = " + String.format("%,.2f", totalLuasPermukaan));
            appendLog("[" + tn + "]         |  Rata-rata Luas      = " + String.format("%,.2f", totalLuas / jumlahData));
            appendLog("[" + tn + "]         |  Rata-rata Volume    = " + String.format("%,.2f", totalVolume / jumlahData));
        }
    }
